...
Comprehensive end-to-end test suite for Jonche Platform API.
Tests all API entry points, authentication, and cross-service communication.
Run from project root: pytest tests/test_e2e_api.py -v
"""
...
class TestIntegrations:
    ...
    def test_complete_admin_flow(self, api_client):
        """Complete admin flow: login → access admin endpoints."""
        # 1. Login as admin
        login_res = api_client.post(
            '/api/auth/admin/login',
            json={"email": "admin@jonche.com", "password": "adminpass"}
        ...
        # 2. Access admin endpoint (if implemented)
    ...
    def test_complete_retailer_flow(self, api_client):
        """Complete retailer flow: login → list retailers."""
        # 1. Login as retailer
        login_res = api_client.post(
            '/api/auth/retailer/login',
            json={"email": "retailer@kith.com", "password": "retailerpass"}
